...
- [ ] **GEG-031** Modes control both **rendering** and **allowed input surfaces**.
  ...
  _Implementation note:_ Staff-only (Mode 2) and Tab-only (Mode 3) are now supported by cropping the shared Staff+Tab SVG via viewBox adjustments (see `applyStaffTabCrop` in `src/app/gameController.ts`).
