...
export function renderOverlay(
  ...
  const sc = ((state.settings as any).surfaceControlsV11?.surfaces ?? null) as any;
  const showPrompt = sc?.fretboard?.prompt?.enabled !== false;
  const showPersistence = sc?.fretboard?.persistence?.enabled !== false;
  ...
  // persistence = claimed marks (ownership dots + claimed labels)
  if (showPersistence) {
    children.push(gDots);
    if (showOwned) children.push(gClaimLabels);
  }

  // reference labels (learning overlay)
  if (showAll) children.push(gAllLabels);

  // prompt = prompt/feedback overlays
  if (showPrompt && gPulse) children.push(gPulse);

  // If everything is hidden, keep an empty group so the SVG remains valid
  if (children.length === 0) children.push(el('g'));
