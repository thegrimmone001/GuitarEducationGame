...
## 2026-01-25 — Canon lock (Phase B-2 + Advanced Menu + Pitch Sets)
- Locked terminology: **Persistence** replaces prior “Display” usage in the current scheme (boards remain always visible).
- Locked Phase B-2 artifacts: Presets A–J definition table + schema notes, plus a v1.1 schema registry addendum for:
  - Surface Controls (Prompt / Mark / Persistence / SAM)
  - Answer Rules (Answer Set vs Answer Count; Octave includes a range)
  - Timers (Turn Timer, Check Clock aka Chess Clock/Time Bank, Runtime Cap, answer-attempt behavior)
  - Pitch Sets (Intervals / Chords / Scales-Modes / Arpeggios)
  - Progressions (mode-aware degrees, overrides, borrowed chords, secondary dominants)
  - Tonal Assist (System tuning + output contract; primary home in Gameplay Misc Info Panel)
- UI direction locked: Advanced Menu uses a responsive, expandable “wing” overlay (no scrolling; layout must remain stable as toggles change).
