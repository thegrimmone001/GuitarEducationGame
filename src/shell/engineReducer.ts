...
function createInitialState(settings: GameSettings, profiles: PlayerProfile[], seed: number): GameState {
  ...
  for (let s = 0; s < 6; s++) {
    for (let f = 0; f < fretCount; f++) {
      ...
      // Open string (fret 0) remains available even when the focused fret range starts at 1.
      const inFretRange = (f === 0) || (f >= normalized.domain.minFret && f <= normalized.domain.maxFret);
...
export function reducer(state: GameState | null, action: Action): { state: GameState; effects: Effect[] } {
  ...
  switch (action.type) {
    ...
    case "DEV_FORCE_PHASE": {
	  ...
	  // In normal gameplay, LAST_CHANCE is entered via end-of-round logic which
	  // computes a per-player queue and sets currentPlayer accordingly.
	  // Dev/Test forcing should replicate that setup; otherwise LAST_CHANCE can
	  // look like it "isn't working".
	  if (action.phase === "LAST_CHANCE") {
	    const orderList: number[] = [];
	    for (let i = 0; i < state.players.length; i++) {
	      if ((state.players[i]?.tokenCount ?? 0) <= 0) continue;
	      if (!playerHasAnyActionablePrompt(state, i)) continue;
	      orderList.push(i);
	    }

	    if (orderList.length === 0) {
	      // No one can act, so keep the match in a normal playable state.
	      state.lastChance.active = false;
	      state.lastChance.order = new Int16Array(0);
    ...
    case "DEV_SET_PROMPT": {
      ...
      // Allowed in dev mode or Learning difficulty (practice workflows).
      const devEnabled = !!state.settings.dev?.enabled;
      const learning = state.settings.difficulty === Difficulty.LEARNING;
      if (!devEnabled && !learning) return { state, effects };

      const vid = action.variantId | 0;
      if (vid < 0 || vid >= state.variants.allowed.length) return { state, effects };
      if (state.variants.allowed[vid] !== 1) return { state, effects };

      state.prompt.variantId = vid;
      effects.push({ type: "PROMPT_CHANGED", variantId: vid });
      return { state, effects };
    }
    ...
    case "START_TURN": {
      ...
      state.prompt.variantId = pick.variantId;
      // UI hint: a new active turn has started for the current player.
// Prompt sequencing: choose a fresh actionable prompt at the start of each turn.
// This re-enables the "random prompt generator" behavior while respecting actionable/claim/steal rules.
{
  const pick = pickNextPromptVariant(state, state.currentPlayer, false);
  state = pick.state;
  if (pick.variantId !== null) state.prompt.variantId = pick.variantId;
}

effects.push({ type: "TURN_STARTED", playerId: state.players[state.currentPlayer]!.profile.id });
effects.push({ type: "PROMPT_CHANGED", variantId: state.prompt.variantId });
return { state, effects };
    }
